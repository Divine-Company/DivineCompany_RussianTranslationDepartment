...
{
    public static class LCB_Russian_Font
    {
        public static List<TMP_FontAsset> tmprussianfonts = new();
        public static List<string> tmprussianfontsnames = new();
        #region Шрифты
        public static bool AddRussianFont(string path)
        {
            ...
            foreach (var Asset in AllAssets)
            {
                ...
                if (TryCastFontAsset)
                {
                    ...
                    tmprussianfonts.Add(TryCastFontAsset);
                    tmprussianfontsnames.Add(TryCastFontAsset.name);
                }
                
            }

            if(tmprussianfonts.Count == 0)
                return false;
            else
                return true;
        ...
        {
            return tmprussianfonts[Math.Min(idx, tmprussianfonts.Count - 1)];
        }

        public static bool IsRussianFont(TMP_FontAsset fontAsset)
            => tmprussianfontsnames.Contains(fontAsset.name);
        ...
        private static bool WriteMaterialProperty(TextMeshProMaterialSetter __instance)
        {
            if (!__instance._fontMaterialInstance)
                return false;
            if (!GetRussianFonts(__instance._text.font.name, out _) && !IsRussianFont(__instance._text.font))
                return true;
        ...
        private static bool GetScenario(string scenarioID, LOCALIZE_LANGUAGE lang, ref Scenario __result)
        {
            ...
            if (!textAsset)
            {
                LCB_DCLCMod.LogError("Story Unknown Error! Call Story: Dirty Hacker");
                scenarioID = "SDUMMY";
            ...
            {
                LCB_DCLCMod.LogError("Story Error! Can't Find <" + scenarioID + "> RU Story File, Use Raw EN Story");
